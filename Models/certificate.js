...
const certificateSchema = new mongoose.Schema({
  ...
  branchId: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'branch',
    required: false // Optional for legacy certificates
  },
  pdfPaths: {
    type: [String],
    default: []
  },
    // NEW: Store GridFS file IDs instead of local path
  pdfFileIds: [{
    type: mongoose.Schema.Types.ObjectId,
...
certificateSchema.pre('save', function(next) {
  ...
  if (this.expiryDate < now) {
    this.status = 'Expired';
  } else if (this.expiryDate <= ninetyDaysFromNow) {

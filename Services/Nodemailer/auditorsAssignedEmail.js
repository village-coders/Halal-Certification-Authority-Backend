...
/**
 * Sends an email to the client when auditors are assigned and prep documents are ready.
 *
 ...
 */
const auditorsAssignedEmail = async (email, clientName, applicationId) => {
  try {
    console.log("📤 Sending auditors assigned email to:", email);
    ...
    const data = await transporter.sendMail({
      ...
      to: email,
      subject: `👥 Auditors Assigned — Application ${applicationId}`,
      html: `
        <div style="font-family: Arial, sans-serif; font-size: 16px; color: #333; line-height: 1.6; padding: 20px; background-color: #f9fafb;">
          <div style="max-width: 600px; margin: auto; background: #ffffff; border: 1px solid #e0e0e0; border-radius: 10px; padding: 30px;">
            ...
            <div style="background-color: #f0fdf4; border-left: 4px solid #00853b; padding: 16px; margin: 24px 0; border-radius: 4px;">
              <p style="margin: 0; font-weight: bold; color: #166534; font-size: 17px;">Auditors Assigned</p>
              <p style="margin: 6px 0 0 0; color: #15803d;">
                Auditors have been assigned to your audit session, and the audit preparation documents are now available.
              </p>
              <p style="margin: 8px 0 0 0; color: #15803d; font-weight: bold;">
                Next Step: <span style="font-weight: normal;">Please log in to your HDI Portal to view and download the audit preparation documents. To arrange the transportation and pick-up of the auditors, kindly contact the Administrative Secretary at 08139143986 or via email at <a href="mailto:support@halalcert.com.ng" style="color: #00853b; text-decoration: underline;">support@halalcert.com.ng</a> to make the necessary arrangements.</span>
              </p>
            </div>
    ...
    console.log("📧 Auditors assigned email sent successfully!");
    console.log("Message ID:", data.messageId);
    return data;
  } catch (error) {
    console.error("❌ Error sending auditors assigned email:", error);
    return null;
  }
};

module.exports = auditorsAssignedEmail;

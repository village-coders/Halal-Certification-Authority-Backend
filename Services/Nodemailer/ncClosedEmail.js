...
/**
 * Sends an email to the client when all NC corrections have been reviewed and verified as closed.
 *
 ...
 */
const ncClosedEmail = async (email, clientName, applicationId) => {
  try {
    console.log("📤 Sending NC Closed email to:", email);
    ...
    const data = await transporter.sendMail({
      ...
      to: email,
      subject: `✅ Non-Conformities Closed — Application ${applicationId}`,
      html: `
        <div style="font-family: Arial, sans-serif; font-size: 16px; color: #333; line-height: 1.6; padding: 20px; background-color: #f9fafb;">
          <div style="max-width: 600px; margin: auto; background: #ffffff; border: 1px solid #e0e0e0; border-radius: 10px; padding: 30px;">
            ...
            <div style="background-color: #f0fdf4; border-left: 4px solid #00853b; padding: 16px; margin: 24px 0; border-radius: 4px;">
              <p style="margin: 0; font-weight: bold; color: #166534; font-size: 17px;">Non-Conformities Closed</p>
              <p style="margin: 6px 0 0 0; color: #15803d;">
                We are pleased to inform you that all Non-Conformity (NC) corrections for your application have been reviewed and verified as closed.
              </p>
              <p style="margin: 8px 0 0 0; color: #15803d; font-weight: bold;">
                Next Step: <span style="font-weight: normal;">The Lead Auditor will submit the Final Audit Report to the Shariah Board for final review and approval.</span>
              </p>
            </div>
    ...
    console.log("📧 NC Closed email sent successfully!");
    console.log("Message ID:", data.messageId);
    return data;
  } catch (error) {
    console.error("❌ Error sending NC Closed email:", error);
    return null;
  }
};

module.exports = ncClosedEmail;

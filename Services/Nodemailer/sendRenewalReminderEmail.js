...
/**
 * Sends an email to the client when an admin sends a certificate renewal/expiring reminder.
 *
 * @param {string} recipientEmail - Client's email address.
 ...
 * @param {boolean} isExpired - Whether the certificate has already expired.
 */
const sendRenewalReminderEmail = async (recipientEmail, companyName, certificateNumber, expiryDate, isExpired) => {
    try {
        console.log("📤 Sending certificate renewal reminder email to:", recipientEmail);

        const formattedExpiryDate = new Date(expiryDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' });
        ...
        const data = await transporter.sendMail({
            ...
            to: recipientEmail,
            subject: `⚠️ Action Required: Certificate ${certificateNumber} ${isExpired ? "has Expired" : "is Expiring Soon"}`,
            html: `
                <div style="font-family: Arial, sans-serif; font-size: 16px; color: #333; line-height: 1.6; padding: 20px; background-color: #f9fafb;">
                  <div style="max-width: 600px; margin: auto; background: #ffffff; border: 1px solid #e0e0e0; border-radius: 10px; padding: 30px;">
                    ...
                    <p>
                      We wish to inform you that your Halal Certificate (<strong>${certificateNumber}</strong>) is scheduled to expire on <strong>${formattedExpiryDate}</strong>. To ensure uninterrupted certification, we recommend commencing the recertification process before the expiry date.
                    </p>

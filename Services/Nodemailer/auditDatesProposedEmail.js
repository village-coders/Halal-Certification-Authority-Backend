...
/**
 * Sends an email to the client when audit dates are proposed.
 *
 ...
 */
const auditDatesProposedEmail = async (email, clientName, applicationId) => {
  try {
    console.log("📤 Sending audit dates proposed email to:", email);
    ...
    const data = await transporter.sendMail({
      ...
      to: email,
      subject: `🗓️ Audit Dates Proposed — Application ${applicationId}`,
      html: `
        <div style="font-family: Arial, sans-serif; font-size: 16px; color: #333; line-height: 1.6; padding: 20px; background-color: #f9fafb;">
          <div style="max-width: 600px; margin: auto; background: #ffffff; border: 1px solid #e0e0e0; border-radius: 10px; padding: 30px;">
            ...
            <div style="background-color: #f0fdf4; border-left: 4px solid #00853b; padding: 16px; margin: 24px 0; border-radius: 4px;">
              <p style="margin: 0; font-weight: bold; color: #166534; font-size: 17px;">Audit Dates Proposed</p>
              <p style="margin: 6px 0 0 0; color: #15803d;">
                Two audit date options have been proposed for your application.
              </p>
              <p style="margin: 8px 0 0 0; color: #15803d; font-weight: bold;">
                Next Step: <span style="font-weight: normal;">Please log in to your HDI Portal to select your preferred audit date or propose alternative dates if the available options are not convenient.</span>
              </p>
            </div>

            <p>
              You can track the progress of your application and view updates by logging into your dashboard.
            </p>
    ...
    console.log("📧 Audit dates proposed email sent successfully!");
    console.log("Message ID:", data.messageId);
    return data;
  } catch (error) {
    console.error("❌ Error sending audit dates proposed email:", error);
    return null;
  }
};

module.exports = auditDatesProposedEmail;

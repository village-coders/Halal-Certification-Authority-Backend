...
/**
 * Sends an email to the client when their application is forwarded to the Shari'a Board after audit completion.
 *
 ...
 */
const sentToShariaBoardEmail = async (email, clientName, applicationId) => {
  try {
    console.log("📤 Sending Application Sent to Sharia Board email to:", email);
    ...
    const data = await transporter.sendMail({
      ...
      to: email,
      subject: `📜 Application Forwarded to Shari'a Board — Application ${applicationId}`,
      html: `
        <div style="font-family: Arial, sans-serif; font-size: 16px; color: #333; line-height: 1.6; padding: 20px; background-color: #f9fafb;">
          <div style="max-width: 600px; margin: auto; background: #ffffff; border: 1px solid #e0e0e0; border-radius: 10px; padding: 30px;">
            ...
            <div style="background-color: #f0fdf4; border-left: 4px solid #00853b; padding: 16px; margin: 24px 0; border-radius: 4px;">
              <p style="margin: 0; font-weight: bold; color: #166534; font-size: 17px;">Audit Phase Completed</p>
              <p style="margin: 6px 0 0 0; color: #15803d;">
                The audit phase for application <strong>${applicationId}</strong> holds finalized completion status.
              </p>
              <p style="margin: 8px 0 0 0; color: #15803d; font-weight: bold;">
                Next Step: <span style="font-weight: normal;">Your application will now be forwarded to the Shari'a Board for review.</span>
              </p>
            </div>

            <p>
              You can track the real-time progress of your application and view details by logging into your dashboard.
            </p>
    ...
    console.log("📧 Sent to Sharia Board email sent successfully!");
    console.log("Message ID:", data.messageId);
    return data;
  } catch (error) {
    console.error("❌ Error sending Sent to Sharia Board email:", error);
    return null;
  }
};

module.exports = sentToShariaBoardEmail;

...
const respondToAudit = async (req, res) => {
    ...
    try {
        ...
        if (status === 'Accepted') {
            ...
            if (newProposed.length !== 3) {
                return res.status(400).json({ message: 'Proposed dates array must have exactly 3 options' });
            }
...
const remindNcCorrection = async (req, res) => {
    ...
    try {
        ...
        // Reusing the corrective action reminder email logic for simplicity
        await sendCorrectiveActionReminderEmail(
            audit.userId.email,
            audit.userId.companyName,
            [{ issue: 'Please upload your NC Correction file in the portal.' }]
        );

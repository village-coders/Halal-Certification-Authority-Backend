...
const updateProcessStep = [processUpload.fields([{ name: 'file', maxCount: 10 }, { name: 'label', maxCount: 10 }, { name: 'prepDoc', maxCount: 3 }]), async (req, res) => {
    const { id } = req.params;
            ...
            case 6: {
                ...
                if (subStepNum === 1 && data) {
                    ...
                    if (auditInfo.action === 'propose') {
                        // Phase 1: Propose 3 audit periods (date + fromTime + toDate)
                        const pDates = auditInfo.proposedDates || [];
                        if (pDates.length !== 3) {
                            throw new Error('Please propose exactly 3 date options');
                        }
                        ...
                        await sendNotification('Audit Dates Proposed', `3 audit date options have been proposed for your application (${application.applicationNumber}). Next Step: Please log in to select your preferred date or propose alternative dates.`);
                    ...
                    // Sub-step 5: NC Report Closed — resolve all corrections
                    const auditId = application.processData?.audit?.auditId;
                    if (auditId) {
                    ...
                    // Sub-step 7: Admin confirms Audit Report Received
                    const auditId = application.processData?.audit?.auditId;
                    if (auditId) {
                        await auditModel.findByIdAndUpdate(auditId, {
                            status: 'Completed',
                    ...
                    application.processData.audit.subStep = 7;

...
const checkAndStatusSync = async () => {
    try {
        ...
        console.error('Error in checkAndStatusSync:', error);
    }
};
...
const getCertificates = async (req, res, next) => {
  try {
    ...
    const companyIds = certificates.map(cert => cert.companyId);

    const companies = await userModel.find({
      registrationNo: { $in: companyIds }
    });

    const companyMap = {};
    companies.forEach(c => {
      companyMap[c.registrationNo] = c.companyName;
    });

    const allCertificates = certificates.map(cert => ({
      ...cert.toObject(),
      companyName: companyMap[cert.companyId] || null,
    }));
...
const getCertificate = async (req, res, next) => {
  try {
    const certificate = await certificateModel.findById(req.params.id)
      ...
      .populate('branchId', 'branchName address city state contactName contactNumber email webAddress');
...
// Generate certificate from accepted application
const generateCertificate = async (req, res, next) => {
  try {
    ...
    if (application.status.toLocaleLowerCase() !== 'accepted') {
      return res.status(400).json({ message: 'Application must be accepted to generate certificate' });
    }
    ...
    const products = await productModel.find({ applicationId: application._id })
...
const downloadCertificate = async (req, res, next) => {
  try {
    ...
    // Authorization check: Only admin or the certificate owner can download
    const isAdmin = req.user.role === 'admin' || req.user.role === 'super admin';
    const isOwner = req.user.registrationNo === certificate.companyId;

    if (!isAdmin && !isOwner) {
      return res.status(403).json({ message: 'You do not have permission to download this certificate' });
    }

    // Check if PDF exists in GridFS (new system: pdfFileIds array)
    if (certificate.pdfFileIds && certificate.pdfFileIds.length > 0) {
      const bucket = getGridFSBucket();
      const fileId = certificate.pdfFileIds[0]; // Download the first PDF
      ...
      downloadStream.on('error', (error) => {
        ...
        res.status(500).json({ message: 'Failed to download certificate from storage' });
      });
      return;
    }

    // Check if PDF exists in GridFS (old system: single pdfFileId)
    ...
    // Fallback: Check if pdfPath exists
    if (certificate.pdfPath) {
      // If it's a full URL (external storage), redirect to it
      if (certificate.pdfPath.startsWith('http')) {
        return res.redirect(certificate.pdfPath);
      }

      // If it's a local file path
      const absolutePath = path.isAbsolute(certificate.pdfPath) 
        ? certificate.pdfPath 
        : path.join(process.cwd(), certificate.pdfPath);

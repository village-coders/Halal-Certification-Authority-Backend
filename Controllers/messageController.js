    ...
    if (receiver === 'admin') {
      try {
        const notification = new notificationModel({
          title: 'New Message',
          message: `${populatedMessage.sender.companyName || populatedMessage.sender.fullName} sent a new message`,
          companyId: userId
        });
...
exports.getUnreadCount = async (req, res) => {
  try {
    ...
    if (req.user.role === 'admin' || req.user.role === 'super admin') {
      query.receiver = { $in: [userId, 'admin'] };
    } else {
...
exports.getAllUserConversations = async (req, res) => {
  try {
    ...
    const pipeline = [
      ...
      {
        $match: {
          'user.role': { $nin: ['admin', 'super admin'] }
        }
      },
